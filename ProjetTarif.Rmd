...
```{r "Data management : Regroupepemnt de modalité, Section du jeu de données en un jeu d'apprentissage et un jeu de test & petites clarifications"}

################################################
######################## Récupération des données et Jonction des deux bases
################################################
...
# Re-factorisons certaines variables :
  str(data.full)
  cols <- c("VehGas")
  data.full[cols] <- lapply(data.full[cols], function(x) factor(x))
  str(data.full)
...
data.full %>% makeplot("VehAge") -> .VehAgeBefore
...
data.full %>% makeplot("VehAge",title = "After",continuous=FALSE) -> .VehAgeAfter 

################################################
######################## Regrouppement de l'Age du conducteur
################################################
# Graphique de l'?tat du monde : 
data.full %>% makeplot("DrivAge") -> .DrivAgeBefore
...
# Graphique de l'?tat du monde Apr?s : 
data.full %>% makeplot("DrivAge",title="After",continuous=FALSE) -> .DrivAgeAfter 
################################################
######################## Regrouppement de la puissance du v?hicule
################################################
data.full %>% makeplot("VehPower") -> .VehPowerBefore 

data.full$VehPower %<>% 
  cut(., breaks = c(4,5,6,7,8,11,15), include.lowest = TRUE,right=FALSE) %T>%
  {print(summary(.))}

data.full %>% makeplot("VehPower",title="After",continuous=FALSE) -> .VehPowerAfter

################################################
######################## Regrouppement d'Area
################################################
data.full %>% makeplot("Area",continuous=FALSE) -> .AreaBefore 

data.full$Area %<>% 
  fct_collapse("EF" = c("E","F")) %T>%
  {print(summary(.))}

data.full %>% makeplot("Area",title="After",continuous=FALSE) -> .AreaAfter
...
data.full %>% makeplot("VehBrand",title="After",continuous=FALSE) -> .VehBrandAfter
...
# paramètre : 
  .Proportion.Wanted = 0.8 # pour des question de rapiditée d'exection, j'ai déscendu la proportion a 0.01, il faut la remonter a 0.8 avent de rendre le code.
...
################################################
  ...
  .MyList = list(data.freq,data.sev,data.full,train.freq,train.sev,train.full,test.freq,test.sev,test.full)
  names(.MyList)=c("data.freq","data.sev","data.full","train.freq","train.sev","train.full","test.freq","test.sev","test.full")
  Databases <-
...
# ajout des modèles fittés a la base de donnée : 
      ...
      # Normal que ce soit pas rapide, ça prend du temps c'est comme ça. 
      # Beuacoup de data, plus d'une 20aine de modèles différents, etc...
      dfMod <- 
...
```{r "Base sévérité : Récupération des données",include=FALSE}
...
selec.freq <- Databases %>% 
  filter(Modele=="freq",Type=="train",Selected==TRUE) %>% .$Mod

selec.sev <- Databases %>% 
  filter(Modele=="sev",Type=="train",Selected==TRUE) %>% .$Mod

# Ok il faurais maintenant récupérer sous forme de fonction les regrouppements de modélité effectué dans les modèles selectionés.
# fais chiiiiiier j'ai passer la nuit dessut j'ai casiment pas avencer...

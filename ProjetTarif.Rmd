...
```{r "Data management : Regroupepemnt de modalité, définition de l'individu de référence, creation du jeu d'apprentissage et du jeu de test & petites clarifications"}
...
mod.freq$poissonLog<- train.full %>% 
...
mod.freq$poissonLog2<- train.full %>% 
...
mod.freq$poissonLog2b<- train.full %>% 
...
mod.freq$poissonLog4 <- train.full %>% 
...
################################################
mod.freq$odPoissonLog<- train.full %>% 
...
mod.freq$odPoissonLog2<- train.full %>% 
...
# mod.freq$ZIPLog <-
#   zeroinfl(data = train.full, formula = ClaimNb ~ Area + VehPower + VehAge + DrivAge + BonusMalus + VehBrand + VehGas + Density + Region, offset=log(Exposure), dist="poisson") %T>%
#   {print(summary(.))}

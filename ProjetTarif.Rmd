...
AddToDb <- function(.database,liste_des_modeles,.Modele,.Type){
      ...
      .database %<>% full_join(dfMod)
      .database[!grepl("Original",.database$Nom),] %<>%
        mutate(data = Mod %>% map("data"))
      # on revois le resultat
      return(.database)
}


Databases %<>% 
  AddToDb(mod.freq,"freq","train") %T>%
  {print(.)}
